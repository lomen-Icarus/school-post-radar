...
async def cmd_stats(message: Message, db: Database, settings: Settings) -> None:
    ...
    rejected = await db.scalar("SELECT COUNT(*) FROM achievement_candidates WHERE review_status = 'rejected'")
    ...
    errors = await db.fetchall(
        ...
        "JOIN communities c USING(community_key) WHERE cur.consecutive_errors > 0 "
        "ORDER BY cur.consecutive_errors DESC LIMIT 8"
    )
    lines = [
        "<b>📈 Статистика</b>",
