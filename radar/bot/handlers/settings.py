...
def build_router() -> Router:
    ...
    async def on_custom_times(
        ...
        try:
            ...
            await message.answer(f"⚠️ {escape(str(exc))}. Пример: <code>09:30, 21:00</code>")
            return
        if len(times) > 6:
    ...
    async def cmd_tz(message: Message, db: Database, subscriber: repo.Subscriber, state: FSMContext) -> None:
        parts = (message.text or "").split(maxsplit=1)
        if len(parts) < 2:
            await state.set_state(ScheduleForm.timezone)  # следующий текст будет принят как часовой пояс
