...
async def build_status(db: Database, settings: Settings, scanner: Scanner, sub: repo.Subscriber) -> str:
    ...
    if runs:
        r = runs[0]
        state = {"running": "выполняется", "done": "завершён", "aborted": "прерван"}.get(r.status, r.status)
        lines.append(
    ...
    lines.append("")
    if sub.mode == "digest":
        lines.append(f"Ваш следующий дайджест: {humanize_local(from_iso(sub.next_digest_at), sub.timezone)}")
    else:
...
async def cb_status(
    ...
    await edit_or_send(query, await build_status(db, settings, scanner, subscriber), keyboards.status_menu())
    await query.answer()
